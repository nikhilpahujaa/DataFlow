...
const Home = () => {
  ...
  const testimonials = [
    {
      ...
      content: "DataFlow saved us weeks of manual migration work. The intelligent recommendations were spot-on!",
      rating: 5,
      avatar: "SJ"
    },
    {
      ...
      content: "The schema analysis feature is incredible. It helped us identify potential issues before migration.",
      rating: 5,
      avatar: "MC"
    },
    {
      ...
      content: "Fast, reliable, and secure. DataFlow made our database migration seamless and worry-free.",
      rating: 5,
      avatar: "ER"
    }
  ];
  ...
  const securityFeatures = [
    { icon: Lock, title: "SSL Encryption", description: "All data transfers are encrypted with industry-standard SSL" },
    ...
    <div className="space-y-6 sm:space-y-8">
      ...
      <div className="pt-4 pb-6 sm:pb-8">
        ...
        <div className="flex flex-col sm:flex-row justify-center items-center space-y-4 sm:space-y-0 sm:space-x-4 lg:space-x-8">
          {/* Source DB */}
          <span className={`px-4 sm:px-6 py-3 rounded-full text-base sm:text-lg font-bold bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300 border-2 border-blue-200 dark:border-blue-700 shadow-sm`}>
            MySQL
          </span>
          ...
          <div className="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-4 lg:space-x-8">
            ...
            <div className="flex flex-col space-y-4 lg:space-y-8">
                <span className={`px-4 sm:px-6 py-3 rounded-full text-base sm:text-lg font-bold bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300 text-center border-2 border-green-200 dark:border-green-700 shadow-sm`}>
                    PostgreSQL
                </span>
                <span className={`px-4 sm:px-6 py-3 rounded-full text-base sm:text-lg font-bold bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-300 text-center border-2 border-purple-200 dark:border-purple-700 shadow-sm`}>
                    MongoDB
                </span>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
        {features.map((feature) => {
          ...
          return (
            <div key={feature.title} className="bg-white dark:bg-gray-800 rounded-lg p-4 sm:p-6 hover:shadow-md transition-all duration-200 transform hover:scale-105 text-center group">
              <div className={`inline-flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 rounded-lg ${feature.bgColor} mb-3 sm:mb-4 group-hover:scale-110 transition-transform duration-200`}>
          ...
          {securityFeatures.map((feature) => {
            ...
            return (
              <div key={feature.title} className="text-center space-y-3">
                ...
                <p className="text-sm text-gray-600 dark:text-gray-300 leading-relaxed max-w-xs mx-auto">{feature.description}</p>
              </div>
            );
          })}
        </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {testimonials.map((testimonial) => (
            <div key={testimonial.name} className="bg-gray-50 dark:bg-gray-700 rounded-lg p-6 space-y-4">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {pricingTiers.map((tier) => (
            <div key={tier.name} className={`relative rounded-lg p-6 border-2 transition-all duration-200 hover:scale-105 flex flex-col ${
              tier.popular 
              ...
              <div className="text-center space-y-4 flex-1 flex flex-col">
                ...
                <div className="space-y-1">
                  <div className="text-3xl font-bold text-gray-900 dark:text-white">
                    {tier.price}
                    {tier.period && <span className="text-lg text-gray-600 dark:text-gray-400">{tier.period}</span>}
                  </div>
